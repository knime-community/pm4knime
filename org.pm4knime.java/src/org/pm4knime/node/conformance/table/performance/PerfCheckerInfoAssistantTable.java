 ...
 *
 */
public class PerfCheckerInfoAssistantTable {
	PerfCounterTable infoProvider;
	SMPerformanceParameter m_parameter;
	ManifestEvClassPatternTable  mResult;
	NumberFormat nfDouble ;
	NumberFormat nfInteger;
	// init it with the counter information

	public PerfCheckerInfoAssistantTable(SMPerformanceParameter parameter, ManifestTable  manifest, PerfCounterTable counter) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		m_parameter = parameter;
		mResult = (ManifestEvClassPatternTable) manifest;
	...
	private void buildInfoParameter() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
		...
		// create caseFilter
		boolean[] caseFilter = new boolean[mResult.getLog().getTraces().size()];
		...
		for (String clfPlusClass : m_parameter.getClassifierSet().getStringArrayValue()) {
			System.out.println("1: " + clfPlusClass);
